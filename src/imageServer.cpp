    ...
    while (this->isOpened())
    {
        ...
        for (std::vector<int>::iterator it = begin; it != end; ++it, ++count)
        {
            if (count > NUM_CONNECTIONS)
                break;
            struct clientPayload *cp = new clientPayload({ (imageServer*)this, { outbuf.data(), outlen, *it } });
            payloads.push_back(cp);
            pthread_create(&threads[count], NULL, &imageServer::clientWrite_Helper, cp);
        }
        for (; count > 0; count--)
        {
...
void
imageServer::ClientWrite(clientFrame & cf)
{
    stringstream head;
